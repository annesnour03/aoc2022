...
void get_new_score(Monkey& m) {
    if (m.operation == '*') {
        ...
        if (m.square)
            ...
            mult = m.amount;
        m.items[0] = (m.items[0] * mult);
        m.items[0] = m.items[0] % divide;
    } else {
        m.items[0] = (m.items[0] + m.amount);
        ;
        m.items[0] = m.items[0] % divide;
    }
}
...
uint64_t solve(std::vector<Monkey> input) {
    for (auto m : input) {
        divide *= m.test_amount;
    }
    ...
    return (uint64_t)input[0].inspected * (uint64_t)input[1].inspected;
}
    ...
    while (std::getline(file, line)) {
        ...
        switch (line_idx) {
            ...
            case 1: {
                ...
                std::vector<long long> my((std::istream_iterator<long long>(iss)), (std::istream_iterator<long long>()));
                m.items = my;
            } break;
