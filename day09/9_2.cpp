...
int solve(std::vector<Instruction> input) {
    ...
    for (int i = 0; i < input.size(); i++) {
        Instruction current_ins = input[i];
        for (int j = 0; j < current_ins.amount; j++) {
            // Simply move the head
            *head = get_new_pos(current_ins.d, *head);
            for (int k = 1; k < length; k++) {
                Position *current = snake[k];
                Position *parent = snake[k - 1];

                // We dont have to move anything.
                if (parent->is_nearby(*current)) continue;
                if (!parent->same_row_or_column(*current)) {
                    Position q = *parent - *current;
                    Position add = {-1, -1};
                    if (q.x > 0)
                        add.x = 1;
